...
function mbPageLoad(){
    ...
    bookingMade.forEach(function(bookingOption){
        if(bookingOption[2] < 12){
            document.getElementById(`mb-room${bookingCounter}`).innerHTML = `${bookingOption[0]}/${bookingOption[2]}:00AM-${bookingOption[2]}:59AM`;
            document.getElementById("mb-room" + bookingCounter).value = bookingCounter;
        }
        else if (bookingOption[2] == 12){
            document.getElementById(`mb-room${bookingCounter}`).innerHTML = `${bookingOption[0]}/${bookingOption[2]}:00PM-${bookingOption[2]}:59PM`;
            document.getElementById("mb-room" + bookingCounter).value = bookingCounter;
        }
        else if (bookingOption[2] > 12){
            document.getElementById(`mb-room${bookingCounter}`).innerHTML = `${bookingOption[0]}/${bookingOption[2]-12}:00PM-${bookingOption[2]-12}:59PM`;
            document.getElementById("mb-room" + bookingCounter).value = bookingCounter;
        }
        else
            console.log("Possible Option Conversion Error.")
        bookingCounter++;
    });
    for(i = 0; i < 20; i++){
...
function bookingSelector(){
    ...
    if(roomSelectorStart < 12){
        ...
        console.log("Possible Table Population Error.")
}
