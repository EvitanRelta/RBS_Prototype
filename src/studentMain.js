    ...
    for(let i = 0; i < 24; i++){
        ...
        if(hourConvert == 2){
            ...
            if(i >= roomSelectorArray[2] && i <= roomSelectorArray[3]){
                ...
                document.getElementById(`td-button-${paddedNum}`).addEventListener("click", handleBooking);
                document.getElementById("td-time-" + paddedNum).setAttribute('class','defaultTable')
            }
            else if(i < roomSelectorArray[2] || i > roomSelectorArray[3]){
                document.getElementById("td-time-" + paddedNum).innerHTML = `-`;
                document.getElementById("td-time-" + paddedNum).setAttribute('class','defaultTable')
            ...
            bookingMade.forEach(function(bookingHolder){
                if(bookingHolder[0] === roomSelectorArray[0] && bookingHolder[2] === i){
                    ...
                    document.getElementById("td-time-" + paddedNum).setAttribute('class','bookedTable')
                }
            });
        }
...
function handleBooking() {
    ...
    if(bookConfirm && currentCredit >= costHour){
        ...
        alert(`You do not have enough credits to book this room ($${(Math.round(currentCredit* 100) / 100).toFixed(2)})\nPlease topup your credits.`);
    }
    else{
        return false;
    }
