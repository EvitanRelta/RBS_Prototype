    ...
    if (selectedIndex !== -1) {
        ...
        roomDetailsData.innerHTML =
            ...
            (selectedRoom.available
                ? 'Reserved (' + selectedRoom.reservationDate + ')'
                : 'Not Reserved')
    }
...
function launchRoom() {
    if (selectedIndex !== -1) {
        ...
        updateOutput(
            'Room launched successfully. Room is now available for reservations on ' +
                reservationDate +
                '.'
        )
        setTimeout(function () {
...
function updateRoomDetailNumber() {
    const roomDetailsHeader = document.querySelector(
        '#roomFunctionsTable tr:last-child th:first-child'
    )
